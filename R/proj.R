...
proj_create <- function(project_root = ".") {
  ...
  .libPaths(unique(c(proj_lib_dir(), .libPaths())))
}

proj_lib_dir <- function() {
...
proj_rprofile <- function() {
  c("# This file is created automatically by `pak:::proj_create()`",
...
proj_load <- function(root = ".", ...) {
  ...
  libpath <- file.path(root, proj_lib_dir())
  ...
  if (length(missing)) {
    stop("Dependencies missing from private library: ",
         ...
         "Call `pak:::proj_install_dev()` to install them.")
  }
  ...
  if (length(loaded <- intersect(loadedNamespaces(), deps$deps))) {
    ...
    if (any(deleted <- ! file.exists(paths))) {
      stop("Some loaded dependencies do not exist on disk: ",
           ...
           "Call `pak:::proj_install_dev()` to reinstall them.")
    }
    ...
    if (any(mismatch)) {
      stop("Packages already loaded from outside the private library: ",
           ...
           "Restart R, and call `pak:::proj_load()` again.")
    }
  }
...
proj_get_missing_deps <- function(root) {
  ...
  status <- lib_status_internal(file.path(root, proj_lib_dir()))
  list(
...
proj_get_dirs <- function(root) {
  ...
  mkdirp(lib <- file.path(root, proj_lib_dir()))
