...
you can use [`repo_add()`](https://pak.r-lib.org/reference/repo_add.md).
...
[`repo_get()`](https://pak.r-lib.org/reference/repo_get.md) also
...
[`repo_get()`](https://pak.r-lib.org/reference/repo_get.md) now does not
show a warning message, and also sets the `has_password` column to
...
The output of
[`repo_status()`](https://pak.r-lib.org/reference/repo_status.md) has
extra columns, compared to
[`repo_get()`](https://pak.r-lib.org/reference/repo_get.md), and it also
has a separate row for each platform. If everything works, then the
`has_password` column is `TRUE` for authenticated repositories, and the
`ok` column is `TRUE` if
[`repo_status()`](https://pak.r-lib.org/reference/repo_status.md) was
able to perform an (authenticated) HTTP HEAD request to the metadata
file of a platform in a repository.

If you need even more information about repo authentication, e.g.
because
[`repo_status()`](https://pak.r-lib.org/reference/repo_status.md) shows
some failures, then use the
[`repo_auth()`](https://pak.r-lib.org/reference/repo_auth.md) function:
...
The output of
[`repo_auth()`](https://pak.r-lib.org/reference/repo_auth.md) has the
following extra columns:
...
E.g. here [`meta_update()`](https://pak.r-lib.org/reference/metadata.md)
